...
# for i in range(9,10):
    ...
    if True:
        ...
        weight_range = np.arange(0, 2, 1)
        pareto_results = []

        for w in weight_range:
            def weighted_pareto_rule(m):
                return w *costs_rule(m) + (1 - w) * emms_rule(m)
            if hasattr(nh3w, 'obj'):
        ...
        pareto_array = np.array(pareto_results)  # shape: (n_weights, 3)
        costs = pareto_array[:, 1]
        emms = pareto_array[:, 2]
        w_idx_08 = np.where(np.isclose(pareto_array[:, 0], 1))[0][0]
        w_idx_02 = np.where(np.isclose(pareto_array[:, 0], 0))[0][0]

        cost_08 = pareto_array[w_idx_08, 2]
        cost_02 = pareto_array[w_idx_02, 2]
        tradeoff_length = abs(cost_08 - cost_02)

        # Initialize lists if they don't exist
        if 'tradeoff_lengths' not in locals():
            tradeoff_lengths = []
            tradeoff_indices = []
            tradeoff_labels = []
        ...
        # Find segments where both cost and emms stay nearly constant
        knee_ranges = []
        start = 0

        for j in range(1, len(costs)):
            if abs(costs[j] - costs[start]) > tol or abs(emms[j] - emms[start]) > tol:
                if j - start > 1:
        ...
        if knee_ranges:
            ...
            print(f"Selected knee point at weight {w_mid} with cost {pareto_array[w_start, 1]} and emms {pareto_array[w_start, 2]}")
        else:
            # Fall back to default if no knee found
            w_mid = 0.5
            print("No knee point found, using default weight 0.5.")

        def selected_pareto_rule(m):
            return w_mid * costs_rule(m) + (1 - w_mid) * (emms_rule(m)+wateruse_rule(m)+safety_rule(m))
...
# # plt.axvspan(0, 121, color=groupingcolor, alpha=groupingalpha)
# # plt.axvspan(144, 178, color=groupingcolor, alpha=groupingalpha)
...
# # ax1.axvspan(competing_region, color=groupingcolor, alpha=groupingalpha)
# # ax1.axvspan(competing_region, color=groupingcolor, alpha=groupingalpha)
# for idx in competing_region:
...
# ax3.plot(time_range, df1["power price"].values[a:b], label="Power Price", color="blue")
# ax3.plot(time_range, df1["CI kgCO2/kwh"].values[a:b], label="CI kgCO2/kWh", color="green")
# # ax3.axvspan(competing_region, color=groupingcolor, alpha=groupingalpha)
# # ax3.axvspan(competing_region, color=groupingcolor, alpha=groupingalpha)
# for idx in competing_region:
...
# plt.figure()
# plt.plot(pareto_array[:, 1], pareto_array[:, 2], marker='o')
# plt.xlabel("Cost")
# plt.ylabel("Emissions")
# plt.title("Pareto Frontier (True_Label == 3)")
# plt.grid(True)
# plt.show()
...
plt.bar(tradeoff_indices, tradeoff_lengths, color=bar_colors, edgecolor='black')
...
# Create legend
legend_handles = [plt.Line2D([0], [0], color=color_map(i), lw=6, label=f"True Label {lbl}")
