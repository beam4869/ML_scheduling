...
# df2 = pd.read_excel("G:/My Drive/AA gourp/machine learning/2024_NewEngland_DA/combining last nine months of New England 2024/Tuned Para Random_forest_last_9_2024_NewEngland_2_classification.xlsx")
df2 = pd.read_csv("/Users/hongxuan/Library/CloudStorage/GoogleDrive-hongxuan@umich.edu/My Drive/AA gourp/machine learning/2024_NewEngland_DA/combining last nine months of New England 2024/lstm_200_two_class_predicted_vs_true_results.csv")
# df2 = pd.read_csv("G:/My Drive/AA gourp/machine learning/2024_NewEngland_DA/combining last nine months of New England 2024/lstm_200_two_class_predicted_vs_true_results.csv")
...
competing_region = []
        # if not all(name in locals() for name in ['tradeoff_lengths1', 'tradeoff_lengths2', 'tradeoff_lengths3', 'tradeoff_lengths4', 'tradeoff_indices', 'tradeoff_labels']):
tradeoff_lengths1 = []
tradeoff_lengths2 = []
tradeoff_lengths3 = []
tradeoff_lengths4 = []
...
# for i in range(9,10):
    ...
    # nh3w.costs = Objective(rule=costs_rule, sense=minimize)
    def emms_rule(m):
    ...
    def Pareto_rule(m):
        # return 0.45*costs_rule(m)+0.55*emms_rule(m) ### make them more balanced
        return 0.45*2*costs_rule(m)+0.55*(emms_rule(m)+wateruse_rule(m)+safety_rule(m)) ### make them more balanced
    def average_rule(m):
        # return 0.45*costs_rule(m)+0.55*emms_rule(m) ### make them more balanced
        return 0.5*3*costs_rule(m)+0.5*emms_rule(m) ### make them more balanced
    # if i in range(0,81) or i in range(104, 138):


    if True:
        # nh3w.obj = Objective(rule=Pareto_rule, sense=minimize)
        ...
        ####indentify knee points#####

        cost_val1 = value(costs_rule(nh3w))
        emms_val1 = value(emms_rule(nh3w))
        water_val1 = value(wateruse_rule(nh3w))
        safety_val1 = value(safety_rule(nh3w))
        ...
        cost_val2 = value(costs_rule(nh3w))
        emms_val2 = value(emms_rule(nh3w))
        water_val2 = value(wateruse_rule(nh3w))
        safety_val2 = value(safety_rule(nh3w))
        # print(f"Cost only cost: {cost_val1}, Cost only Emissions : {emms_val1}")
        # print(f"Emissions only cost: {cost_val2}, Emissions only Emissions : {emms_val2}")
        # pareto_results.append(abs(cost_val1-cost_val2)))
        tradeoff_length1 = abs(cost_val1 - cost_val2)
        tradeoff_length2 = abs(emms_val2 - emms_val1)
        tradeoff_length3 = abs(water_val2 - water_val1)
        tradeoff_length4 = abs(safety_val2 - safety_val1)

        # Initialize lists if they don't exist
        # Store values for plotting
        tradeoff_lengths1.append(tradeoff_length1)
        tradeoff_lengths2.append(tradeoff_length2)
        tradeoff_lengths3.append(tradeoff_length3)
        tradeoff_lengths4.append(tradeoff_length4)
        tradeoff_indices.append(i)
        tradeoff_labels.append(true_labels[i])


    else:
        nh3w.obj = Objective(rule=average_rule, sense=minimize)
        # nh3w.obj = Objective(rule=costs_rule, sense=minimize)
        # print("No competing region, using cost objective")
        # nh3w.obj = Objective(rule=emms_rule, sense=minimize)
    # nh3w.obj = Objective(rule=costs_rule, sense=minimize)
    # nh3w.obj = Objective(rule=emms_rule, sense=minimize)
    # if i in range(0,8) or i in range(55,56) or i in range(56,57):
    #     nh3w.obj = Objective(rule=Pareto_rule, sense=minimize)
...
print(tradeoff_indices)
print(tradeoff_labels)
print(tradeoff_lengths1)
print(tradeoff_lengths2)
print(tradeoff_lengths3)
print(tradeoff_lengths4)

import matplotlib.cm as cm

# Normalize labels to colormap range (for binary or multi-class)
unique_labels = sorted(set(tradeoff_labels))
color_map = cm.get_cmap('tab10', len(unique_labels))
label_to_color = {label: color_map(i) for i, label in enumerate(unique_labels)}
bar_colors = [label_to_color[lbl] for lbl in tradeoff_labels]

# Create bar plot
plt.figure(figsize=(16, 2.5))
plt.bar(tradeoff_indices, tradeoff_lengths1, color=bar_colors, edgecolor='black')
plt.xlabel("Time Index (Competing Region)")
plt.ylabel("Cost Tradeoff Length\n(Cost at w=0.8 - Cost at w=0.2)")
plt.title("Tradeoff Region Length Across Time Steps (Color-Coded by True Label)")
plt.ylim(bottom=-800)
# for idx in competing_region:
#     plt.axvspan(idx, idx+1, color=groupingcolor, alpha=groupingalpha)

# Create legend
label_names = {0: "Correlating", 1: "Competing"}
legend_handles = [plt.Line2D([0], [0], color=color_map(i), lw=6, label=label_names.get(lbl, f"Label {lbl}"))
                  for i, lbl in enumerate(unique_labels)]
plt.legend(handles=legend_handles, title="Region Type")
# plt.grid(axis='y')
plt.tight_layout()
plt.show()
# objective_df = pd.DataFrame(ObjectiveNumber, columns=["Costs", "Emissions", "Water Use", "Safety"])

# # Save the DataFrame to a CSV file
# output_path = "Cost_optimal_ObjectiveNumber.csv"  # Specify the desired output path
# objective_df.to_csv(output_path, index=False)

# print(f"ObjectiveNumber saved to {output_path}"

# Create bar plot
plt.figure(figsize=(16, 2.5))
plt.bar(tradeoff_indices, tradeoff_lengths2, color=bar_colors, edgecolor='black')
plt.xlabel("Time Index (Competing Region)")
plt.ylabel("Cost Tradeoff Length\n(Cost at w=0.8 - Cost at w=0.2)")
plt.title("Tradeoff Region Length Across Time Steps (Color-Coded by True Label)")
plt.ylim(bottom=-800)
...
# Create bar plot
plt.figure(figsize=(16, 2.5))
plt.bar(tradeoff_indices, tradeoff_lengths3, color=bar_colors, edgecolor='black')
plt.xlabel("Time Index (Competing Region)")
plt.ylabel("Cost Tradeoff Length\n(Cost at w=0.8 - Cost at w=0.2)")
plt.title("Tradeoff Region Length Across Time Steps (Color-Coded by True Label)")
plt.ylim(bottom=-800)
...
# Create bar plot
plt.figure(figsize=(16, 2.5))
plt.bar(tradeoff_indices, tradeoff_lengths4, color=bar_colors, edgecolor='black')
plt.xlabel("Time Index",fontsize=18)
plt.ylabel("Tradeoff Length", fontsize=18)
plt.title("Tradeoff Region Length Across Time Steps (Color-Coded by True Label)")
plt.ylim(bottom=-5)
