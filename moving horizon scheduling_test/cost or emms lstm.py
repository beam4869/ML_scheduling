...
df2 = pd.read_csv("/Users/hongxuan/Library/CloudStorage/GoogleDrive-hongxuan@umich.edu/My Drive/AA gourp/machine learning/2024_NewEngland_DA/lstm_predicted_vs_true_results.csv")
...
gridemm_total = df1["CI kgCO2/kwh"].values[320:540+47]
lstm_predicted = df2["Predicted_Label"].values[320:540]
...
# for i in range(9,10):
    ...
    def Pareto_rule3(m):
        ...
        return 0.45*3*costs_rule(m)+0.55*(emms_rule(m)+wateruse_rule(m)+safety_rule(m)) ### make them more balanced
    def Pareto_rule1(m):
        # return 0.45*costs_rule(m)+0.55*emms_rule(m) ### make them more balanced
        return 0.45*3*(costs_rule(m)+safety_rule(m))+0.55*(emms_rule(m)+wateruse_rule(m))
    # if i in range(0,81) or i in range(104, 138):
    # if i in range(0,121) or i in range(144, 178):
    ...
    # nh3w.cost = Objective(rule=costs_rule, sense=minimize)
    # nh3w.cost = Objective(rule=emms_rule, sense=minimize)
    # if i in range(9,18) or i in range(26,31) or i in range(37,42) or i in range(53,57) or i in range(58,59) or i in range(77,84):
    if lstm_predicted[i] == 3:
        nh3w.obj = Objective(rule=Pareto_rule3, sense=minimize)
    elif lstm_predicted[i] == 1:
        nh3w.obj = Objective(rule=Pareto_rule1, sense=minimize)
    else:
        nh3w.obj = Objective(rule=costs_rule, sense=minimize)
        # nh3w.obj = Objective(rule=emms_rule, sense=minimize)
