...
# df2 = pd.read_csv("G:/My Drive/AA gourp/machine learning/2024_NewEngland_DA/lstm_predicted_vs_true_results.csv")
df2 = pd.read_excel("G:/My Drive/AA gourp/machine learning/2024_NewEngland_DA/combining last nine months of New England 2024/Random_forest_last_9_2024_NewEngland_two_classification.xlsx")
...
# lstm_predicted = df2["Predicted_Label"].values[a:b]
Random_forest_predicted = df2["Predicted_Label"].values[a:b]
true_labels = df2["True_Label"].values[a:b]
labels = Random_forest_predicted
...
# for i in range(9,10):
    ...
    if labels[i] == 3 or labels[i] == 1 or labels[i] == 2:
...
# and the power-price / carbon-intensity profiles plotted in the bottom subplot
...
# Per-step labels (the RF predicted label drives the grey-shaded regions here)
labels_df = pd.DataFrame({
    "Time_h":                  x_hours,
    "True_Label":              true_labels,
    "RF_Predicted_Label":      Random_forest_predicted,
    "Label_Used_For_Routing":  labels,
})
...
summary_df = pd.DataFrame({
    ...
    "Value": [
        ...
        float(np.sum(totalcost) / max(np.sum(mnh3), 1e-12)),
        float(np.sum(totalemms) / max(np.sum(mnh3), 1e-12)),
        a,
        b,
        movhor_tl,
    ],
...
excel_out = "ml_classification_optimal_source_data.xlsx"
