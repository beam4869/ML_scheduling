...
# ==========================================================
# NEW Plot 3: tradeoff-length bar plot color-coded by true labels
# ==========================================================
if len(tradeoff_lengths) > 0:
    ...
    tradeoff_df = pd.DataFrame({
        ...
        "emms_at_w_0_2": tradeoff_emms_02,
        "emms_at_w_0_8": tradeoff_emms_08
    })

    print("\nTradeoff length results:")
    print(tradeoff_df)

    # Optional: save to CSV
    tradeoff_df.to_csv("tradeoff_length_by_true_label.csv", index=False)
    print("Saved tradeoff results to tradeoff_length_by_true_label.csv")

    unique_labels = sorted(tradeoff_df["true_label"].unique())
    ...
    bar_colors = [
        ...
        for label in tradeoff_df["true_label"]
    ]

    plt.figure(figsize=(18, 4))

    plt.bar(
        tradeoff_df["time_index"],
        tradeoff_df["tradeoff_length_abs"],
        color=bar_colors,
        edgecolor="black",
        linewidth=0.5
    )

    plt.xlabel("Moving Horizon Time Index")
    plt.ylabel(r"Tradeoff Length: $|Cost_{w=0.8} - Cost_{w=0.2}|$")
    plt.title("Cost Tradeoff Length from Weighted-Sum Pareto Sweep")
    ...
    plt.bar(
        tradeoff_df["time_index"],
        tradeoff_df["cost_0_8_minus_cost_0_2"],
        color=bar_colors,
        edgecolor="black",
        linewidth=0.5
    )
    ...
    plt.xlabel("Moving Horizon Time Index")
    plt.ylabel(r"$Cost_{w=0.8} - Cost_{w=0.2}$")
    plt.title("Signed Cost Difference from Weighted-Sum Pareto Sweep")
    ...
    print("No predicted competing regions were found, so no tradeoff-length bar plot was generated.")
