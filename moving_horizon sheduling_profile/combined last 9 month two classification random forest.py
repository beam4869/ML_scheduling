...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    model = RandomForestClassifier(random_state=42,bootstrap= False, class_weight= None, criterion = 'entropy', max_depth= None, max_features= 0.6, min_samples_leaf= 1, min_samples_split= 6, n_estimators= 1219)
...
new_df = pd.read_excel("G:\\My Drive\\AA gourp\\machine learning\\2024_NewEngland_DA\\combining last nine months of New England 2024\\test set.xlsx")  # Shape should be (n_samples, 96)
X_new = new_df.iloc[:, 96:105]
X_new_scaled = scaler.transform(X_new)
y_new_pred = model.predict(X_new_scaled)
y_true = new_df.iloc[:, 107].values
print('Truth ', y_true)
...
output_df.to_excel('Random_forest_last_9_2024_NewEngland_5_classification.xlsx', index=False)