...
# ==========================================
# Paths — data lives one level up in datasets/
# ==========================================
...
# ==========================================
# 2. Identify Case B (Chlor LOW / Ammonia HIGH)
# ==========================================
...
println("Computing Chlor-Alkali Pareto frontier for Case B (window $idx_B)...")
pc_chlor, pe_chlor, I_mat_B = chlor_pareto(price_B, gridCI_B; n_pts=N_PTS_CHLOR)

println("Computing Ammonia Pareto frontier for Case B (window $idx_B)...")
...
#    corner anchors.
#
#    "Pure cost" anchor   = visible upper-left corner
#                           (min cost, tiebreak: max emission)
#    "Pure emission"      = visible lower-right corner
#                           (min emission, tiebreak: max cost)
# ==========================================
...
scatter!(p1, [new_strength[idx_A]], [old_strength[idx_A]],
    label="Case A (Chlor↑ NH₃↓)", color=:royalblue,
    markersize=9, markershape=:star5)
scatter!(p1, [new_strength[idx_B]], [old_strength[idx_B]],
    label="Case B (Chlor↓ NH₃↑)", color=:crimson,
    markersize=9, markershape=:star5)
...
#     Split into 4 vertically-stacked sub-panels, one per Pareto point.
#     The 4 panels are connected (margins=0 between them); only the
#     bottom panel keeps the x-axis; only one panel carries the y-unit.
#     Colour identifies the Pareto point (matches panel b).
# ==========================================
p3_subs = Plots.Plot[]
for (i, idx) in enumerate(i4_ammo)
    is_first = (i == 1)
...
# ==========================================
# 10. p4 — Case B price & emission dual-axis  (Row 2, Col 1)
# ==========================================
p4 = plot(time_axis, price_B,
    xlabel="Time Step (h)",
    ylabel="Price (\$/kWh)",
    title="(d) Case B: Price & Emission",
...
#     Matches case_study_analysis.jl: plain scatter of all weighted-sum
#     solutions, no connecting line. 4 cost-quartile points highlighted.
# ==========================================
...
#     Same connected-stack layout as panel (c).
# ==========================================
p6_subs = Plots.Plot[]
for (i, idx) in enumerate(i4_chlor)
    is_first = (i == 1)
    is_last  = (i == 4)
    is_unit  = (i == 2)
    # bottom two panels (Intermediate 2 + Pure emission) have spikes
    # on the right side → move legend to top-left to avoid overlap.
...
fig = plot(p1, p2, p3_subs[1], p3_subs[2], p3_subs[3], p3_subs[4],
    ...
    layout = lay,
    size   = (2000, 1200),              # taller canvas so the 4-strip stacks grow vertically
    left_margin   = 12Plots.mm,
    bottom_margin = 8Plots.mm,
    top_margin    = 6Plots.mm,
    right_margin  = 12Plots.mm,
    # plot_title    = "Ammonia vs Chlor-Alkali — Case B  (window $idx_B)",
    plot_titlefontsize = 13)

out_path = joinpath(@__DIR__, "combined_comparison_caseB.png")
