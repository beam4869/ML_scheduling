using  StatsBase, CSV, DataFrames, CSV
...
df = DataFrame(CSV.read("G:\\My Drive\\AA gourp\\machine learning\\2024_NewEngland_DA\\2024NewEngland_test_dataset.csv", DataFrame))
groupings = df[!,"grouping"][1:8712]
groupingNum = zeros(length(groupings)).+9
for i in 1:length(groupings)
    if groupings[i] == "[[1, 2], [3, 4]]" || groupings[i] == "[[3, 4], [1, 2]]"
        groupingNum[i] = 0
    elseif  groupings[i] == "[[1, 4], [2, 3]]" || groupings[i] == "[[2, 3], [1, 4]]"
        groupingNum[i] = 1
    elseif groupings[i] == "[[1], [2, 3, 4]]"
        groupingNum[i] = 2
    elseif groupings[i] == "[[2], [1, 3, 4]]"
        groupingNum[i] = 3
    elseif groupings[i] == "[[4], [1, 2, 3]]"
        groupingNum[i] = 4
    end
end
df2 = DataFrame(groupingNums = groupingNum)
CSV.write("2024NewEngland_groupingNum.csv", df2)
