...
df1 = DataFrame(XLSX.readtable("/Users/hongxuan/Library/CloudStorage/GoogleDrive-hongxuan@umich.edu/My Drive/AA gourp/machine learning/datasets for ML/dataset of Jan Apr Aug Oct 2022 LA DA/Jan LA 2022 price and emission.xlsx","Sheet1"))
...
LAJanMatrixforML = zeros(28*24,96)
# TokyoMatrixforML[:,97] = df2[!,"Column1"]
A = df1[!,"Jan_price_total_LA"][1:720]#entering the first line of the row to let it find it
# ceb_total = convert(Array{Float64,1}, A)
B = df1[!,"Jan_emission_total_LA"][1:720]
...
# gridemm=gridemm_total[n+24*m-24:n+m*24+23]
    LAJanMatrixforML[n+24*m-24,1:48] = convert(Array{Float64,1}, A[n+24*m-24:n+m*24+23])
    LAJanMatrixforML[n+24*m-24,49:96] = convert(Array{Float64,1}, B[n+24*m-24:n+m*24+23])
    end
end     
...
df = hcat(df3,df1[:,17][1:672],makeunique=true)
CSV.write("Dataset_of_2022_Jan_LA_DA_for_ML.csv", df)